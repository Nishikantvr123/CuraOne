...
export const submitQuiz = async (req, res, next) => {
  try {
    ...
    const existingProfile = await findOne('doshaProfiles', { userId: req.user.id });
    if (existingProfile) {
      await updateOne('doshaProfiles', { id: existingProfile.id }, {
        ...profile,
...
export const getMyDoshaProfile = async (req, res, next) => {
  try {
    const profile = await findOne('doshaProfiles', { userId: req.user.id });
