...
// @access Public
import { generateOTP, storePending, getPending, sendOTPEmail } from '../services/otpService.js';
...
export const sendOtp = async (req, res, next) => {
  try {
    ...
    const existing = getPending(email);
    if (existing && Date.now() - existing.createdAt < 60_000) {
      const wait = Math.ceil((60_000 - (Date.now() - existing.createdAt)) / 1000);
    ...
    storePending(email, otp, {
      ...
      isActive: true,
      lastLogin: null,
      preferences: { notifications: { email: true, sms: false, push: true }, language: 'en', timezone: 'UTC' }
    });
...
// @access Public
import { verifyOTP, deletePending } from '../services/otpService.js';
...
export const verifyOtp = async (req, res, next) => {
  try {
    ...
    const newUser = await create('users', result.userData);
    deletePending(email);
