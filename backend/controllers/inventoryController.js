...
export const getInventory = async (req, res, next) => {
  try {
    ...
    let items = await findMany('inventory', {}, {
      sortBy: 'name',
      sortOrder: 'asc'
    });
...
export const getLowStockItems = async (req, res, next) => {
  try {
    ...
    const items = await findMany('inventory');
...
export const createInventoryItem = async (req, res, next) => {
  try {
    ...
    const existingItem = await findOne('inventory', { name: name.trim() });
    if (existingItem) {
      res.status(400);
...
export const updateInventoryItem = async (req, res, next) => {
  try {
    const item = await findOne('inventory', { id: req.params.id });
    ...
    const updatedItem = await updateOne('inventory', { id: req.params.id }, {
      ...(name && { name: name.trim() }),
      ...(category && { category }),
