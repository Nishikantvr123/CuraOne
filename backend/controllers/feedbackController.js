...
export const createFeedback = async (req, res, next) => {
  try {
    ...
    const session = await findOne('bookings', { 
    ...
    const existingFeedback = await findOne('feedback', { 
    ...
    // Create feedback
    const feedback = await create('feedback', {
      patientId: req.user.id,
      sessionId: feedbackData.sessionId,
...
export const getFeedback = async (req, res, next) => {
  try {
    ...
    const allFeedback = await find('feedback', query);
    const startIndex = (page - 1) * parseInt(limit);
    const endIndex = startIndex + parseInt(limit);
    const feedback = allFeedback.slice(startIndex, endIndex);

    // Populate related data
    const feedbackWithDetails = await Promise.all(feedback.map(async (fb) => {
      const patient = await findOne('users', { id: fb.patientId });
      const practitioner = await findOne('users', { id: fb.practitionerId });
      const session = await findOne('bookings', { id: fb.sessionId });
      const therapy = await findOne('therapies', { id: fb.therapyId });
...
export const getFeedbackById = async (req, res, next) => {
  try {
    const feedback = await findOne('feedback', { id: req.params.id });
    ...
    // Populate related data
    const patient = await findOne('users', { id: feedback.patientId });
    const practitioner = await findOne('users', { id: feedback.practitionerId });
    const session = await findOne('bookings', { id: feedback.sessionId });
    const therapy = await findOne('therapies', { id: feedback.therapyId });
...
export const updateFeedback = async (req, res, next) => {
  try {
    ...
    const updatedFeedback = await update('feedback', req.params.id, updateData);
...
export const deleteFeedback = async (req, res, next) => {
  try {
    ...
    await deleteOne('feedback', { id: req.params.id });
...
export const getFeedbackStats = async (req, res, next) => {
  try {
    ...
    const feedback = await find('feedback', {
