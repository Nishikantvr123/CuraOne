...
export const getPatientProgress = async (req, res) => {
  try {
    ...
    const bookings = await findMany('bookings', { patientId });
...
export const getUpcomingSessions = async (req, res) => {
  try {
    ...
    const allBookings = await findMany('bookings', { patientId });
    const now = new Date();
    ...
    const upcomingSessions = await Promise.all(
      allBookings
        ...
        .map(async (booking) => {
          ...
          const practitioner = await findOne('users', { id: booking.practitionerId, role: 'practitioner' });
          return {
            id: booking.id,
...
export const getWellnessData = async (req, res) => {
  try {
    ...
    const wellnessCheckins = await findMany('wellness_checkins', { patientId });
