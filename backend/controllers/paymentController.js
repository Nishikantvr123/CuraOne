...
export const processWebhook = async (req, res, next) => {
  try {
    ...
    const payment = await findOne('payments', { orderId });
    if (!payment) {
      res.status(404);
    ...
    const updatedPayment = await updateOne('payments', { id: payment.id }, {
    ...
    if (isSuccess && payment.bookingId) {
      await updateOne('bookings', { id: payment.bookingId }, {
        paymentStatus: 'paid',
...
export const getMyPayments = async (req, res, next) => {
  try {
    ...
    let payments = await findMany('payments', { userId: req.user.id }, {
      sortBy: 'createdAt',
      sortOrder: 'desc'
    });
...
export const getPaymentById = async (req, res, next) => {
  try {
    const payment = await findOne('payments', { id: req.params.id });
