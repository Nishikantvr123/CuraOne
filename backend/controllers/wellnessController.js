...
export const createWellnessCheckIn = async (req, res, next) => {
  try {
    ...
    const existingCheckIn = await findOne('wellness_checkins', {
      userId: req.user.id,
      date: today
    });
    ...
    const newCheckIn = await create('wellness_checkins', {
      userId: req.user.id,
      date: today,
    ...
    const updatedCheckIn = await update('wellness_checkins', newCheckIn.id, {
      wellnessScore
    });
...
export const getWellnessHistory = async (req, res, next) => {
  try {
    ...
    const allCheckIns = (await find('wellness_checkins', query))
...
export const getWellnessStats = async (req, res, next) => {
  try {
    ...
    const checkIns = (await find('wellness_checkins', {
      userId: req.user.id,
...
export const updateWellnessCheckIn = async (req, res, next) => {
  try {
    const checkIn = await findOne('wellness_checkins', { id: req.params.id });
    ...
    const updatedCheckIn = await update('wellness_checkins', req.params.id, updateData);

    // Recalculate wellness score
    const wellnessScore = calculateWellnessScore(updatedCheckIn);
    const finalCheckIn = await update('wellness_checkins', req.params.id, {
      wellnessScore
    });
...
export const deleteWellnessCheckIn = async (req, res, next) => {
  try {
    ...
    await deleteOne('wellness_checkins', { id: req.params.id });
