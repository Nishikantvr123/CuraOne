...
export const createPrescription = async (req, res, next) => {
  try {
    ...
    const patient = await findOne('users', { id: patientId, role: 'patient' });
    if (!patient) {
      res.status(404);
      throw new Error('Patient not found');
    }

    const inventoryItems = await findMany('inventory');
    ...
    for (const med of medications) {
      ...
      if (inventoryItem) {
        ...
        await updateOne('inventory', { id: inventoryItem.id }, {
...
export const getMyPrescriptions = async (req, res, next) => {
  try {
    ...
    let prescriptions = await findMany('prescriptions', query, {
      sortBy: 'createdAt',
      sortOrder: 'desc'
    });
...
export const getPrescriptionById = async (req, res, next) => {
  try {
    const prescription = await findOne('prescriptions', { id: req.params.id });
...
export const updatePrescription = async (req, res, next) => {
  try {
    ...
    const updatedPrescription = await updateOne('prescriptions', { id: req.params.id }, {
      ...(status && { status }),
