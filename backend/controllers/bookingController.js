...
export const createBooking = async (req, res, next) => {
  try {
    ...
    const therapy = await findOne('therapies', { id: bookingData.therapyId });
    if (!therapy) {
      res.status(404);
    ...
    const practitioner = await findOne('users', { 
    ...
    // Create booking
    const newBooking = await create('bookings', {
      patientId: req.user.id,
      practitionerId: bookingData.practitionerId,
    ...
    const fullBooking = await getBookingWithDetails(newBooking.id);
...
export const getBookings = async (req, res, next) => {
  try {
    ...
    const allBookings = await find('bookings', query);
    const startIndex = (page - 1) * parseInt(limit);
    const endIndex = startIndex + parseInt(limit);
    const bookings = allBookings.slice(startIndex, endIndex);

    // Get full booking details
    const bookingsWithDetails = await Promise.all(bookings.map(booking => getBookingWithDetails(booking.id)));
...
export const getBookingById = async (req, res, next) => {
  try {
    const booking = await findOne('bookings', { id: req.params.id });
...
export const updateBooking = async (req, res, next) => {
  try {
    ...
    const updatedBooking = await update('bookings', req.params.id, updateData);
    const bookingWithDetails = await getBookingWithDetails(updatedBooking.id);

    // Send notification if status changed
    if (req.body.status && req.body.status !== booking.status) {
      const patient = await findOne('users', { id: booking.patientId });
      await sendNotification({
...
export const cancelBooking = async (req, res, next) => {
  try {
    ...
    const updatedBooking = await update('bookings', req.params.id, {
      status: 'cancelled',
    ...
    const notificationUser = await findOne('users', { id: notificationRecipient });
    await sendNotification({
    ...
    const bookingWithDetails2 = await getBookingWithDetails(updatedBooking.id);

    res.json({
      success: true,
      message: 'Booking cancelled successfully',
      data: { booking: bookingWithDetails2 }
    });
  } catch (error) {
    next(error);
  }
...
export const confirmBooking = async (req, res, next) => {
  try {
    ...
    const bookingWithDetails3 = await getBookingWithDetails(updatedBooking.id);

    res.json({
      success: true,
      message: 'Booking confirmed successfully',
      data: { booking: bookingWithDetails3 }
    });
  } catch (error) {
    next(error);
  }
...
export const rescheduleBooking = async (req, res, next) => {
  try {
    ...
    // Check for conflicts
    const conflictingBooking = await findOne('bookings', {
    ...
    // Send notification to practitioner
    const practitioner = await findOne('users', { id: booking.practitionerId });
    await sendNotification({
      recipientId: practitioner.id,
    ...
    const bookingWithDetails4 = await getBookingWithDetails(updatedBooking.id);

    res.json({
      success: true,
      message: 'Booking rescheduled successfully',
      data: { booking: bookingWithDetails4 }
    });
  } catch (error) {
    next(error);
  }
...
export const getAvailableSlots = async (req, res, next) => {
  try {
    ...
    const existingBookings = await find('bookings', {
...
const getBookingWithDetails = async (bookingId) => {
  ...
  const patient = await findOne('users', { id: booking.patientId });
  const practitioner = await findOne('users', { id: booking.practitionerId });
  const therapy = await findOne('therapies', { id: booking.therapyId });
