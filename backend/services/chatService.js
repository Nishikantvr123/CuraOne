...
const getAvailableSlots = async () => {
  const practitioners = await findMany('users', { role: 'practitioner', isActive: true });
...
const checkRateLimit = async (userId) => {
  ...
  const recentChats = (await findMany('chats', { userId }))
...
export const getChatHistory = async (userId, limit = 50) => {
  const chats = await findMany('chats', { userId }, {
    sortBy: 'timestamp',
    sortOrder: 'desc',
    limit
  });
